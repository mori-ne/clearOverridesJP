...
function start() {
    var px = {
        projectName: 'すべてのオーバーライドを消去',
        version: '2024-12-11-v1.2',

        // Default GUI configuration
        processText: true,
        clearParagraphStyle: '全て',
        clearCharacterStyle: '全て',
        processTables: false,
        clearTableStyle: '全て',
...
function processDok(args) {
    ...
    if (px.processObjects) {
        ...
        for (var i = 0; i < results.length; i++) {
            ...
            if (px.clearObjectStyle === '全て' || object.appliedObjectStyle.id === px.clearObjectStyle.id) {
    ...
    if (px.processTables) {
        ...
        for (var i = 0; i < results.length; i++) {
            ...
            if (px.clearTableStyle === '全て' || table.appliedTableStyle.id === px.clearTableStyle.id) {
...
function getConfig(dok, px) {
    ...
    textPanel = mainWindow.add('Panel', undefined, '文字・段落');
    textPanel.alignment = ['fill', 'fill'];
    textPanel.margins = [10, 20, 10, 10];
    textPanel.alignChildren = ['left', 'top'];
    var processTextCheckBox = textPanel.add('Checkbox', undefined, '文字スタイルのオーバーライドを消去');
    ...
    var paragraphStyleText = textParagraphStyleGroup.add('statictext', undefined, '段落スタイル');
    ...
    var item = paragraphStyleDropDown.add('item', '全て');
    item.paragraphStyle = '全て';
    item.selected = true;
    ...
    var characterStyleText = textCharacterStyleGroup.add('statictext', undefined, '文字スタイル');
    ...
    var item = characterStyleDropDown.add('item', '全て');
    item.characterStyle = '全て';
    item.selected = true;
    ...
    tablePanel = mainWindow.add('Panel', undefined, 'テーブル');
    tablePanel.alignment = ['fill', 'fill'];
    tablePanel.margins = [10, 20, 10, 10];
    tablePanel.alignChildren = ['left', 'top'];
    var processTableCheckBox = tablePanel.add('Checkbox', undefined, '表スタイルのオーバーライドを消去');
    ...
    var tableStyleTable = tableStyleGroup.add('statictext', undefined, '表スタイル');
    ...
    var item = tableStyleDropDown.add('item', '全て');
    item.tableStyle = '全て';
    item.selected = true;
    ...
    objectPanel = mainWindow.add('Panel', undefined, 'オブジェクト');
    objectPanel.alignment = ['fill', 'fill'];
    objectPanel.margins = [10, 20, 10, 10];
    objectPanel.alignChildren = ['left', 'top'];
    var processObjectCheckBox = objectPanel.add('Checkbox', undefined, 'オブジェクトスタイルのオーバーライドを消去');
    ...
    var objectStyleObject = objectStyleGroup.add('statictext', undefined, 'オブジェクトスタイル');
    ...
    var item = objectStyleDropDown.add('item', '全て');
    item.selected = true;
    item.objectStyle = '全て';
    ...
    controlGroup.alignment = 'fill';
    with (controlGroup) {
        var goToWebsite = controlGroup.add('button', undefined, 'by grefel / 和訳: morine');
        var divider = controlGroup.add('panel');
        divider.alignment = 'fill';
        var cancelButton = controlGroup.add('button', undefined, 'キャンセル');
        var okButton = controlGroup.add('button', undefined, 'オーバーライドを消去', { name: 'ok' });
    }
...
function openURL(url) {
    if ($.os[0] === 'M') {
        // Mac
        app.doScript('場所を開く "' + url + '"', ScriptLanguage.APPLESCRIPT_LANGUAGE);
    } else {
        // Windows
